...
Se expone el campo "curso" y el dato "incuba" como uno de los entretantos hipotéticos valores adicionales. Lo mismo con el segundo parámetro.

Si bien no sé si es rebuscada mi duda, ya que una lista visual también expone qué campos hay en una tabla, quisiera también ofrecer una posible solución, a forma de autorespuesta: seguiría utilizando Query Params para consultas sencillas o de un único campo, mientras que para consultas más complejas, crearía directamente un buscador dinámico con diferentes tipos de filtros mediante GET para realizar las consultas vía HTTP.
